...
from aresis.service import run_and_store_scan
from aresis.worker.celery_app import celery_app


@celery_app.task(name="aresis.run_scan")
def run_scan_task(subject_id: str) -> str:
    return run_and_store_scan(subject_id)


@celery_app.task(name="aresis.prune_expired")
